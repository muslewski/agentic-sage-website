...
# This is the ONLY way images enter public/assets/ — see "Images" in README.md.
#
#   Usage:   scripts/optimize-image.sh <source-image> <output-basename> <max-width-px>
#   Example: scripts/optimize-image.sh explorations/assets/sage-banner.png sage-banner 1600
#
# Writes public/assets/<basename>.avif and public/assets/<basename>.webp, each resized to
# <max-width-px> (never upscaled). Pick width ≈ 2× the largest CSS size the image renders at.
# Keep the source PNG master in explorations/assets/; reference the output with the <picture>

#!/usr/bin/env node
/**
 * Build beautiful multi-page docs from the product package docs/ tree.
 * Source of truth: sibling agentic-sage/docs (override with DOCS_ROOT).
...
function mdToHref(mdRel) {
  ...
  // recipes/index → recipes/  (not recipes/index/)
...
function renderSidebar(sidebar, currentHref, depth = 0) {
  let html = depth === 0 ? `<nav class="docs-nav" aria-label="Documentation">` : `<ul class="docs-nav-group">`;
  for (const item of sidebar) {
    ...
    const active = currentHref === md || (md === "index.md" && currentHref === "index.md");
...
/** Rewrite .md links to pretty /docs/ URLs */
function rewriteMdLinks(html) {
  return html
    .replace(/href="(\.\.\/)*([a-zA-Z0-9_./-]+)\.md(#[^"]*)?"/g, (_, _up, path, hash) => {
      // external-ish relative to parent repo files — leave as github
      if (path.includes("agentic-sage-mind") || path.startsWith("../")) {
        return `href="https://github.com/muslewski/agentic-sage/blob/main/${path.replace(/^(\.\.\/)+/, "")}.md${hash || ""}"`;
      }
      const pretty = mdToHref(path.endsWith(".md") ? path : `${path}.md`.replace(".md.md", ".md"));
      // path may not have .md in capture - we stripped
...
function pageShell({ title, description, version, sidebarHtml, bodyHtml, currentPath }) {
  const pageTitle = `${title} · SAGE docs`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <link rel="canonical" href="https://sage.muslewski.com${currentPath}">
  ...
  <meta property="og:url" content="https://sage.muslewski.com${currentPath}">
  <meta property="og:image" content="https://sage.muslewski.com/og.jpg">
  <meta name="twitter:card" content="summary_large_image">
  <style>html,body{background:#0E0D0C;color:#EFE7D8;}</style>
  <link rel="stylesheet" href="/src/style.css">
  <link rel="stylesheet" href="/src/docs.css">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Anton&family=Inter:ital,wght@0,400;0,500;0,600;1,400&family=Caveat:wght@400;600;700&family=JetBrains+Mono:ital,wght@0,300;0,400;0,600;0,700;1,400&display=swap" rel="stylesheet">
</head>
<body class="docs-body">
<a class="docs-skip" href="#docs-main">Skip to content</a>

<header class="docs-top paper-tex nav-paper">
  <div class="docs-top-inner">
    <a class="docs-brand" href="/">
      <img src="/favicon.svg" alt="" width="28" height="28">
      <span class="docs-brand-word">SAGE</span>
    ...
    <div class="docs-top-actions">
      ...
      <a class="btn-gold docs-gh" href="https://github.com/muslewski/agentic-sage" target="_blank" rel="noopener">GitHub →</a>
    </div>
  </div>
</header>
...
<div class="docs-shell">
  <aside class="docs-aside">
    ...
    <div class="docs-aside-foot">
      <a href="https://github.com/muslewski/agentic-sage/edit/main/docs/">Edit on GitHub →</a>
    </div>
  </aside>
  ...
  <main id="docs-main" class="docs-main">
    ...
    <footer class="docs-page-foot">
      <p>Source of truth: <code>agentic-sage/docs/</code> · rendered for <strong>v${esc(version)}</strong></p>
      <p class="docs-doctrine">I don't do the work. <span class="gold-glow">I judge it.</span></p>
    </footer>
  </main>
</div>
...
// Override fence to use our highlight (markdown-it wraps in pre>code by default otherwise)
...
for (const href of hrefs) {
  ...
  // Fix parent-repo links like ../../SETUP.md
  bodyHtml = bodyHtml.replace(
    /href="\.\.\/\.\.\/(SETUP|AGENTS|ADAPTERS|CONVENTIONS|README|CHANGELOG|SCHEMA)\.md"/g,
    'href="https://github.com/muslewski/agentic-sage/blob/main/$1.md"',
  );
  bodyHtml = bodyHtml.replace(
    /href="\.\.\/(SETUP|AGENTS|ADAPTERS|CONVENTIONS|README|CHANGELOG|SCHEMA)\.md"/g,
    'href="https://github.com/muslewski/agentic-sage/blob/main/$1.md"',
  ...
  // Write pretty paths: index → generated/docs/index.html
  // getting-started → generated/docs/getting-started/index.html
  // recipes/index.md → generated/docs/recipes/index.html
  let outFile;
...
// Mobile menu inject - add button to shell via post-process (already in shell as optional)
// Add menu button: patch pages to include toggle
...
writeFileSync(
  ...
  JSON.stringify({ version, pages, docsRoot: DOCS_ROOT, builtAt: new Date().toISOString() }, null, 2),
);

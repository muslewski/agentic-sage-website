...
- Before working in an area, read `agentic-sage-site-mind/map/index.md`, then the relevant `map/zones/<slug>.md`.
...
- Route spec-writing output to `agentic-sage-site-mind/specs/` and plan-writing output to `agentic-sage-site-mind/plans/`; keep each note's `summary` field crisp — retrieval engines surface the summary plus one section, not the whole note.

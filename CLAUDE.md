<!-- atlas:onramp v0.1 -->
### Working with the Atlas (`agentic-sage-site-mind/`)

`agentic-sage-site-mind/` is this repository's knowledge base — an Obsidian-compatible
vault that is the single source of *understanding*, kept separate from the
code it describes.

- **Orient Atlas-first.** Before working in an area, read
  `agentic-sage-site-mind/map/index.md`, then the relevant
...
- **Pipeline.** Route spec-writing output to `agentic-sage-site-mind/specs/` and
  plan-writing output to `agentic-sage-site-mind/plans/`.
